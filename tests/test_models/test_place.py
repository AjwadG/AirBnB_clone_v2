...
class test_Place(test_basemodel):
    ...
    def test_reviews_property_in_non_db_storage(self):
        # Test the reviews property in non-DB storage
        place = Place(name='TestPlace')
        review = Review(text='TestReview', place=place)

        # Add the review directly to the storage
        BaseModel.__objects = {'Review.{}'.format(review.id): review}

        # Check if the property returns the correct list of reviews
        self.assertIsInstance(place.reviews, list)
